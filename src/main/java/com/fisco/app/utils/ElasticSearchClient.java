...
import com.fisco.app.entity.ElasticSearchConfig;
import com.fisco.app.entity.TxPackage;
...
/*    将区块数据插入ES中
    public void InsertBlockData(String contractName, BlockData blockData) throws IOException {
        contractName+="_block";
        HashMap<String,Object> data=new HashMap<>();
        data.put("block_height",blockData.getBlockHeight());
        data.put("ip",blockData.getIp());
        data.put("port",blockData.getPort());
        data.put("index",blockData.getIndex());
        data.put("uuid",blockData.getUuid());
        data.put("hash",blockData.getHash());
        IndexRequest request=new IndexRequest(contractName.toLowerCase());
        request.timeout(TimeValue.timeValueSeconds(1));
        request.timeout("1s");
        request.source(data, XContentType.JSON);
        IndexResponse index=restHighLevelClient.index(request, RequestOptions.DEFAULT);

...
public interface TxDetailMapper extends BaseMapper<TxDetailVO> {
    @DS("slave_0")
    @Select("select uid,hash,description from g2_DetailCRUD_insert_method0 where tx_hash = #{txHash}")
    List<TxDetailVO> selectDetailVO0(String txHash);

    @DS("slave_1")
    @Select("select uid,hash,description from g2_DetailCRUD_insert_method1 where tx_hash = #{txHash}")

...
contract TestCRUD {
    ...
    constructor() public {
        ...
        tableFactory.createTable(TABLE_name, "uid", "hash,desc");
    }
    ...
    returns (string[] memory, string[] memory, string[] memory)
    {
        ...
        string[] memory desc_list = new string[](uint256(entries.size()));
        ...
        for (int256 i = 0; i < entries.size(); ++i) {
            ...
            desc_list[uint256(i)] = entry.getString("desc");
            
        }

        return (goods_uid_bytes_list, hash_list, desc_list);
    }
    
     //insert records
    function insert(string memory uid, string memory hash, string memory desc)
    public
    returns (int256)
    {
        Table table = tableFactory.openTable(TABLE_name);
    ...
    //update records
    function update(string memory uid, string memory hash, string memory desc)
    public
    returns (int256)
    {
        Table table = tableFactory.openTable(TABLE_name);
